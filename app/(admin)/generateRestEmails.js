    ...
    <ScrollView className="flex-1 px-4 py-6">
      <View className="mx-auto w-full max-w-2xl">
        <View className="mb-4 rounded-lg border border-gray-200 bg-white p-4">
          <Text className="mb-2 text-sm font-medium text-gray-700">البريد الإلكتروني للمستخدم</Text>
          <TextInput
            value={email}
        ...
        {result && (
          <View
            ...
            <Text className={`mb-2 font-bold ${result.success ? 'text-blue-800' : 'text-red-800'}`}>
              {result.message}
            </Text>

            {result.url && (
              <View>
                <Text className="mb-2 font-bold text-green-700">تم إنشاء رابط إعادة التعيين:</Text>
                <View className="mb-3 rounded-lg border border-gray-200 bg-gray-50 p-3">
                  <Text className="text-sm text-gray-900" selectable={true}>
                    {result.url}
                  </Text>
                </View>
                <TouchableOpacity
                  ...
                  <Text className="text-center font-bold text-white">نسخ الرابط إلى الحافظة</Text>
                </TouchableOpacity>
              </View>
            )}
          </View>
        ...
        <View className="mt-6 rounded-lg border border-gray-200 bg-gray-50 p-4">
          <Text className="mb-3 font-bold text-gray-800">كيفية العمل:</Text>
          <View className="space-y-2">
            <View className="flex-row items-start">
              <Text className="ml-2 text-gray-900">•</Text>
              <Text className="flex-1 text-gray-600">أدخل عنوان البريد الإلكتروني للمستخدم</Text>
            </View>
            <View className="flex-row items-start">
              <Text className="ml-2 text-gray-900">•</Text>
            ...
            <View className="flex-row items-start">
              <Text className="ml-2 text-gray-900">•</Text>
              <Text className="flex-1 text-gray-600">يمكنك نسخ الرابط ومشاركته مع المستخدم</Text>
            </View>
            <View className="flex-row items-start">
              <Text className="ml-2 text-gray-900">•</Text>
              <Text className="flex-1 text-gray-600">
