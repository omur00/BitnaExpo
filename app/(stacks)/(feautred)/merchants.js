...
export default function FeaturedMerchantsPage() {
  ...
  const { data, loading, error, fetchMore, refetch } = useQuery(GET_ALL_FEATURED_MERCHANTS, {
    variables: {
  ...
  const merchants = data?.getAllFeaturedMerchants?.merchants || [];
  const hasNextPage = data?.getAllFeaturedMerchants?.hasNextPage || false;
  const totalCount = data?.getAllFeaturedMerchants?.totalCount || 0;

  const loadMoreMerchants = useCallback(async () => {
    // Prevent multiple calls
    if (!hasNextPage || isLoadingMore || isFetchingRef.current || merchants.length === 0) {
      return;
    }
    ...
    try {
      ...
      const newOffset = offset + merchants.length;
      ...
      const { data: moreData } = await fetchMore({
        ...
        updateQuery: (prevResult, { fetchMoreResult }) => {
          ...
          return {
            getAllFeaturedMerchants: {
              ...fetchMoreResult.getAllFeaturedMerchants,
              merchants: [
                ...prevResult.getAllFeaturedMerchants.merchants,
                ...fetchMoreResult.getAllFeaturedMerchants.merchants,
              ],
            },
          };
        },
      ...
      if (moreData?.getAllFeaturedMerchants?.merchants) {
        setOffset(newOffset);
      }
    } catch (error) {
  ...
  }, [hasNextPage, isLoadingMore, merchants.length, offset, fetchMore]);
  ...
  const renderMerchantCard = ({ item: merchant, index }) => (
    <TouchableOpacity
          ...
          <View className="flex-row items-center gap-3">
            <View className="flex-row items-center gap-1">
              ...
              <Text className="font-inter text-xs text-[#7A8699]">
                {merchant.category?.nameAr || 'غير محدد'}
              </Text>
            </View>
            <View className="flex-row items-center gap-1">
  ...
  const renderFooter = () => {
    if (!hasNextPage && merchants.length > 0) {
      return (
        <View className="py-4">
          <Text className="text-center text-sm text-[#7A8699]">
            تم عرض جميع الجهات المميزة ({merchants.length} من {totalCount})
          </Text>
        </View>
      );
    }
    ...
    return (
      <View className="flex-1 items-center justify-center py-12">
        <View className="mb-4 h-20 w-20 items-center justify-center rounded-full bg-[#F7F9FA]">
          <Ionicons name="storefront-outline" size={32} color="#CBD0D6" />
        </View>
        <Text className="font-poppins mb-2 text-xl font-bold text-[#18344A]">
          لا توجد جهات مميزة حالياً
        </Text>
        <Text className="font-inter mb-6 text-center text-sm text-[#4E6882]">
          لم يتم إضافة أي جهات مميزة بعد. سيتم إضافتها قريباً.
        </Text>
      </View>
    );
  };
  ...
  if (error) {
    return (
      <View className="flex-1 items-center justify-center bg-white p-4">
        ...
        <Text className="font-inter mb-6 text-center text-sm text-[#4E6882]">
          {error.message || 'حدث خطأ في تحميل الجهات المميزة'}
        </Text>
